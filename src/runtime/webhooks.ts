 ...
 *
 * `extract` does verification AND parsing in one call: it reads the three
 ...
 * `v1,<sig>` in the header. Any match → `JSON.parse(body)` and return the event.
 * No match → `WebhookSignatureError`. It NEVER returns an unverified event.
 *
